...
func GetDefault() ConfigClient {
	return ConfigClient{
		...
		Mode:      config.MakeSelect("Cipher Block Chaining (CBC)", []string{"Cipher Block Chaining (CBC)", "Cipher Feedback (CFB)", "Counter (CTR)", "Output Feedback (OFB)"}, config.Display{Description: "Select the mode of operation"}),
