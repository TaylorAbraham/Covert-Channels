...
func GetDefault() ConfigClient {
	return ConfigClient{
		...
		Embedder:     config.MakeSelect("sequence", []string{"sequence", "id", "urgptr", "urgflg", "timestamp", "ecn", "temporal", "ecntemporal"}, config.Display{Description: "The encoding mechanism to use for this protocol.", Name: "Encoding", Group: "Settings"}),
	}
}
